...
export default function JoinBoard() {
  ...
  useEffect(() => {
    const fetchBoard = async () => {
      try {
        const res = await fetch(`${API}/whiteboards/join/${code}`);
  ...
  const handleRequestAccess = async () => {
    ...
    try {
      await apiFetch(`/whiteboards/${board._id}/request-access`, {
        method: "POST",
      });
  ...
  return (
    <div className="relative w-screen h-screen overflow-hidden">
      {/* Board name - top left */}
      <div
        className="absolute top-3 left-3 z-10 bg-background border border-border-muted 
        rounded-md px-3 py-1 text-sm text-foreground flex items-center gap-2"
      >
        <span>{board.name}</span>
        <span
          className="text-xs text-foreground-muted bg-background-highlight 
      ...
      {/* Request to edit - top right */}
      <div className="absolute top-3 right-4 z-10">
        {requestStatus === "idle" && (
          <button
            onClick={handleRequestAccess}
            className="bg-primary text-background text-sm px-4 py-2 rounded-md 
        ...
        {requestStatus === "sent" && (
          <span
            className="text-sm bg-background border border-border-muted 
            px-4 py-2 rounded-md text-foreground-muted"
          >
